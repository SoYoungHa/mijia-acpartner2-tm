...
bool MiioDevice::GetPower(double& outWatts) {
    // 老款 miIO: get_prop ["load_power"] -> 返回 [851.0]
    // 适用于 lumi.acpartner.mcn02 等老款空调伴侣/智能插座
    std::string result;
    if (!Send("get_prop", "[\"load_power\"]", result)) return false;
    // result 形如 [851.0] / [851] / [""] / []；错误响应不含 '['
    auto lb = result.find('[');
    if (lb == std::string::npos) return false;        // 非结果数组（错误响应）
    size_t i = lb + 1;
    while (i < result.size() && result[i] == ' ') i++;
    if (i >= result.size() || result[i] == ']' || result[i] == '"') {
        outWatts = 0.0;                               // 空值（如关机）按 0W
        return true;
    }
